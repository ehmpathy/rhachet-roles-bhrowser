...
describe('browser.snapshot.meta', () => {
  given('[case1] with active browser', () => {
    ...
    when('[t0] skill is invoked', () => {
      then('it captures page metadata', () => {
        const result = skillFull(
          `browser.snapshot meta --tab 0 --url 'chrome://new-tab-page/' --session ${session}`,
        );
  ...
  given('[case2] without --tab', () => {
    when('[t0] skill is invoked without --tab', () => {
      then('it fails with usage message', () => {
        const result = skillNoThrow('browser.snapshot meta');
  ...
  given('[case4] --help', () => {
    when('[t0] skill is invoked with --help', () => {
      then('shows help', () => {
        const result = skillNoThrow('browser.snapshot.meta --help');
