...
if browser_has_help_flag "$@"; then
  browser_emit_help \
    ...
    "capture localStorage and sessionStorage from browser tab" \
    "browser.snapshot storage --focused | --tab N --url 'pattern'" \
...
npx tsx "$SKILL_DIR/browser.snapshot.storage.ts" \
